...
head({[],[]}) -> nil;
head({[H | _], _}) -> H;
head({[], Rear}) -> lists:last(Rear).
    

%% put add documentation here
-spec tail({list(), list()}) -> any().
tail({[], []}) -> nil;
tail({Front, []}) -> lists:last(Front);
tail({_, [H | _]}) -> H.
...
dequeue({[], []}) -> {[], []};
dequeue({[], Rear}) -> 
    [_ | T] = lists:reverse(Rear),
    {T, []};
dequeue({[_ | []], Rear}) -> {lists:reverse(Rear), []};
dequeue({[_ | T], Rear}) -> {T, Rear}.


-spec enqueue_front({list(), list()}, any()) -> {list(), list()}.
enqueue_front({Front, Rear}, N) -> 
    {[N | Front], Rear}.
...
dequeue_back({[], []}) -> {[], []};
dequeue_back({Front, []}) -> 
    {lists:sublist(Front, length(Front) - 1),[]};
dequeue_back({Front, [_ | Tail]}) -> {Front, Tail}.
